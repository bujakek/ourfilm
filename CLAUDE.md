...
- `STRIPE_SECRET_KEY` in `apps/web/.env.local` is an **`sk_test_`** key. Production
...
- **Roles are live; Stripe is live in production and in test mode locally.**
  `apps/web/.env.local` has the four `STRIPE_*` keys in test mode, so a host can run a
...
it, somebody should find out. The queue in `apps/web/lib/upload-queue.ts` knows every
outcome, and until this it told a `console.error` in the guest's own browser.
`apps/web/lib/telemetry.ts` names every event; each carries the event id and, where
...
that finished, a row that exists. `reportServerEvent` in `apps/web/lib/telemetry-server.ts`
...
It was adopted under conditions, and `apps/web/tests/unit/telemetry.test.ts` pins each
...
- **Behind our own origin.** The client talks to `/ingest`, which
  `apps/web/next.config.mjs` rewrites to PostHog. That keeps `connect-src 'self'` in
  ...
  `/ingest/e/` and Next's redirect ran before rewrites — and `apps/web/proxy.ts` issues
...
way in September 2026 before `apps/web/lib/blob-bytes.ts` existed. **The copy alone
...
`apps/web/lib/photo-urls.ts` builds `/storage/v1/object/public/…` from the path with no
...
null` — the grant's own `reason`, not a flat `'grant'`, because an Early
  ...
  clause up without changing. `apps/web/lib/plan-copy.ts` turns it into the host's line;
