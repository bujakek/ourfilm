 ...
 *
 * The counterpart to the web app's `lib/exif.ts` reader, and the reason both
 * exist. Upload strips
...
/**
 * An APP1 segment carrying the capture time and nothing else, from an
 * already-rendered stamp (`2026:08:15 14:32:10`) and offset (`+02:00`).
 *
 ...
 *
 * Takes strings, not an instant and a zone, on purpose: this module is the
 * one piece of the export shared by the web app, the host's browser and the
 * export worker, and the zone-aware rendering stays on the web side (see
 * `lib/format.ts` there). Pure bytes in, pure bytes out.
 */
