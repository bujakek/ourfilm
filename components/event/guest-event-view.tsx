...
export function GuestEventView({
  ...
  /**
   * How many frames are already on the strip.
   *
   * A ref because the uploader reads it from callbacks that run long after the
   * render that produced the number — and writes it from an effect, never
   * during render, which is the distinction `react-hooks/refs` enforces.
   */
  const framesRef = useRef(frames.length)
  useEffect(() => {
   ...
   */
  const claimCell = useCallback((id: string, file: File) => {
    setCaptures((current) => {
      if (current.some((c) => c.id === id)) return current
      const landed = framesRef.current
      return [
        ...current,
        {
          id,
          ...
          index: landed + current.filter((c) => landed <= c.index).length,
          progress: 0,
          confirmed: false,
        },
      ]
   ...
   *
   * Built in an effect rather than during render because it is handed
   * `framesRef`, and a ref read during render is a ref that can disagree with
   * what was painted. Everything it closes over besides that ref is stable for
   * the life of the component: the locale is the event's, and the router is
   * Next's own singleton.
   */
    ...
    const queue = (queueRef.current ??= createUploadQueue({
      ...
      handlers: {
        ...
        onConfirmed(id, shotsRemaining) {
          // State lands first and the animation is a consequence: the counter
          // is set to what the server returned and the odometer rolls from an
          // already-correct value. Commits are strictly ordered because the
          // queue is drained one at a time, so a later commit can never
          // overwrite this count with a staler one. Nothing here is gated on an
          // animation finishing — `onAnimationComplete` never fires while the
          // tab is hidden, and this tab has just spent a minute behind a camera.
        ...
        onDropped(id, reason, silent) {
          ...
          // whole queue rather than once per photo.
          if (reason === 'refused' || silent) return
          setFlash(
            en
        ...
        onAbandoned(ids) {
          ...
          setCaptures((current) => current.filter((c) => !doomed.has(c.id)))
        },
        onRefusal(refusal, { restored }) {
          if (refusal === 'no_shots') setRemaining(0)
          // A shot recovered from a tab the guest does not remember losing gets
          // no error about the roll or the window. "Shooting has ended." is
          // true, and telling someone that when they open the page the next
          // morning names nothing they did and nothing they can fix. The three
          // refusals that *are* actionable still speak.
          if (
            restored &&
            (refusal === 'ended' ||
              refusal === 'not_started' ||
