...
describe('billing details', () => {
  ...
  it('rejects a non-Hungarian invoice address', () => {
    const result = billingDetailsFromStripeSession(
      session({
        address: {
          city: 'Wien',
    ...
    // A Hungarian AAM invoice describes a Hungarian sale. Accepting a foreign
    // address would be a VAT decision, not a harmless UI option.
    if (!result.success) expect(result.error).toContain('magyarországi')
  })
})
