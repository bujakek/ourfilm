...
const sections: LegalSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'A házigazda bejelentkezéséhez a Supabase Auth feltétlenül szükséges munkamenetsütijei kellenek. Az esemény létrehozása előtt a beállítások egy legfeljebb 7 napig élő piszkozatként a böngésző localStorage tárhelyén maradnak, hogy a belépési kör után folytatható legyen a folyamat. Ha a nyelvválasztóval nyelvet váltasz, a választásodat egy legfeljebb egy évig élő, funkcionális sütiben (ourfilm_locale) megjegyezzük, hogy a főoldal legközelebb ezen a nyelven nyíljon meg; más célra nem használjuk.',
...
const englishSections: LegalSection[] = [
  ...
  },
  {
    ...
    body: [
      'Joining sets one strictly necessary, event-specific httpOnly session cookie for up to one year. Supabase Auth uses strictly necessary session cookies for hosts. Before account creation, an event draft stays in the browser’s localStorage for up to seven days. If you change language with the language switcher, a functional cookie (ourfilm_locale) remembers that choice for up to one year so the homepage opens in that language next time; it is used for nothing else. We do not use advertising cookies. Vercel Web Analytics measures aggregate page views without cookies. Our PostHog configuration does not set PostHog cookies or write to browser localStorage or sessionStorage. PostHog nevertheless receives individual technical events from which aggregate reports and error diagnostics are produced, and those events may be pseudonymous data. We do not send image files, photo content, guest display names or email addresses to PostHog.',
    ],
  },
  {
    title: 'Your rights',
